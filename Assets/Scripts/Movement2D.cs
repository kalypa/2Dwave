...
public class Movement2D : MonoBehaviour
{
    ...
    [SerializeField]
    private float xMoveSpeed = 2; // 좌/우 이동 속도
    [SerializeField]
    private float xDelta = 2; // 좌/우 변동 값(이동 범위)
    private float xStartPosition; // x축 시작 위치

    [Header("Vertical Movement")]
    [SerializeField]
    private float yMoveSpeed = 0.5f; // 전진 이동 속도
    ...
    public void MoveToY()
    {
        ...
        // Update(), FixedUpdate() 호출되는 이벤트 메소드에 따라 이동 폭이 크게 바뀐다
